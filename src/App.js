...
import loader from './assets/integrateLoader.gif'
import './App.css';
...
function App() {
  ...
  const [columnDefs] = useState([
    { headerName: "Phase", field: 'phase', cellRenderer: agPhaseCellRendered, width: 120, maxWidth: 150 },
    { headerName: "File Uploaded Time", field: 'uploaded_time', width: 240, maxWidth: 250 },
    { headerName: "Completed Time", field: 'completed_time', width: 240, maxWidth: 250 },
    { headerName: "Error Code", field: 'error_code'},
  ]);
  function agPhaseCellRendered(props) {
    switch (props.value) {
      case "completed":
        return <div><span className='agPhaseBadge completed'>{props.value}</span></div>
      case "processing":
        return <div><span className='agPhaseBadge processing'>{props.value}</span></div>
      case "uploading":
        return <div><span className='agPhaseBadge uploading'>{props.value}</span></div>
      case "validating":
        return <div><span className='agPhaseBadge validating'>{props.value}</span></div>
      case "starting":
        return <div><span className='agPhaseBadge starting'>{props.value}</span></div>
      case "error":
        return <div><span className='agPhaseBadge error'>{props.value}</span></div>
      default:
